use super::{
    vte_goto_instruction, FloatingPanesStack, ImageRenderBundle, KittyImageChunk,
...
impl ImageOutput {
    ...
    fn add_sixel_image_chunks_to_client(
        ...
        if let Some(character_cell_size) = character_cell_size {
            let mut sixel_chunks = if let Some(floating_panes_stack) = floating_panes_stack {
    ...
    fn add_sixel_image_chunks_to_multiple_clients(
        ...
        let character_cell_size = *self.character_cell_size.borrow();
        if let Some(character_cell_size) = character_cell_size {
            let sixel_chunks = if let Some(floating_panes_stack) = floating_panes_stack {
                floating_panes_stack.visible_sixel_image_chunks(
                    sixel_image_chunks,
                    z_index,
                    &character_cell_size,
    ...
    fn add_kitty_image_chunks_to_client(
        ...
        let mut kitty_chunks = if let Some(floating_panes_stack) = floating_panes_stack {
    ...
    fn add_kitty_image_chunks_to_multiple_clients(
        ...
        let kitty_chunks = if let Some(floating_panes_stack) = floating_panes_stack {
            floating_panes_stack.visible_kitty_image_chunks(kitty_image_chunks, z_index)
        } else {
            kitty_image_chunks
        };
    ...
    pub fn add_damage_redraw_image_render_bundle_to_client(
        ...
        self.add_sixel_image_chunks_to_client(
            client_id,
